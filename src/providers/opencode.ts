 ...
 * OPENCODE_* env vars and ANTHROPIC_BASE_URL tell the CLI which provider/model
 * to use at runtime — read from .env on the host (since the host process is
 * launched by launchd/systemd which does not auto-load .env), then injected
 * into the container. NO_PROXY / no_proxy are
